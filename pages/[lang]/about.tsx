...
const Page: NextPage<HomeProps> = (props) => {
  ...
  const [thresholdTime] = useState(100);
  const [stop, setStop] = useState(false);

  const maxSlides = 5;
  const handleOnWheel = (event: any) => {
    if (active >= 0 && event.deltaY < 0) {
      console.log("scrolling up");
      setActive((old) => old - 1);
    } else if (active <= maxSlides && event.deltaY > 0) {
    ...
    setStop(true);
    const isTouchPad =
      event.nativeEvent.wheelDeltaY <= 50 &&
      event.nativeEvent.wheelDeltaY >= -50
        ? true
        : false;

    if (isTouchPad) {
      setTimeout(() => setStop(false), thresholdTime * 5);
    } else {
      setTimeout(() => setStop(false), thresholdTime);
    }
  };
  ...
  return !props.isMobile ||
    ...
    >
      ...
      <div
        onWheel={(e) => !stop && handleOnWheel(e)}
        style={{
          display: "flex",
          height: "100%",
      ...
      >
        ...
        >
          ...
          <section
            className={
              index.firstSection + " " + index.grid + " frameBottomTop"
            }
          >
            ...
            <div className={index.right + " " + index.verticalMargin}>
              ...
              <div className={index.ul}>
                <ReactMarkdown
                  source={pageFromCMS.first_section?.content_text}
                />
              </div>
            </div>
          </section>
          <section
            className={
              index.secondSection + " " + index.grid + " frameBottomTop"
            }
          >
          ...
          >
            <div
              className={
                index.clients + " content-frame " + index.verticalMargin
              }
            >
              <h2
                data-swiper-parallax="500"
          ...
          >
            ...
            <div className={index.right + " " + index.verticalMargin}>
              ...
              <div className={index.ul}>
                <ReactMarkdown
                  source={pageFromCMS.fifth_section?.content_text}
                />
              </div>
            </div>
          </section>
